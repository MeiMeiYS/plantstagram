...
const UserMenu = ({ showUserMenu, setShowUserMenu }) => {
    ...
    useEffect(() => {
        ...
        const closeMenu = e => {
            ...
            if ( profileBtn?.current?.contains(e.target) ||
                settingBtn?.current?.contains(e.target)){
                    setShowUserMenu(false);
            }
        };
    ...
    return (
        <>
            <div className={`user-drop-down ${ showUserMenu && "user-drop-down-open" }`} ref={dropDownMenu}>
                <div id='user-drop-down-arrow'></div>
                <div>
                    <NavLink exact to={`/${username}`} ref={profileBtn}>
